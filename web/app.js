...
function initControls() {
  // POPIA toggle
  const popiaBtn = document.getElementById("popiaToggle");
  if (popiaBtn) {
    popiaBtn.addEventListener("click", () => {
      isPiiMasked = !isPiiMasked;
      popiaBtn.className = `popia-pill ${isPiiMasked ? 'active' : 'inactive'}`;
      const popiaText = document.getElementById("popiaText");
      if (popiaText) {
        popiaText.textContent = isPiiMasked ? "POPIA Masked" : "PII Unmasked (Exec)";
      }
