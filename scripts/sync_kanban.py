...
PRD_PATH = os.path.join(WORKSPACE_DIR, "thin-web-app", "PRD_ARCHITECTURE_REALIGNMENT.md")
...
def main():
    ...
    print(f"[OK] Wrote Kanban data: {JSON_OUTPUT_PATH} ({len(data['tickets'])} tickets)")
    ...
    if os.path.exists(HTML_OUTPUT_PATH):
        ...
        if pattern.search(html):
            ...
            with open(HTML_OUTPUT_PATH, "w", encoding="utf-8") as f:
                f.write(new_html)
            print(f"[OK] Updated inline snapshot in {HTML_OUTPUT_PATH}")
